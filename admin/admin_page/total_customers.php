...
<table class="table  card-body  table-hover table-borderless   border rounded-top rounded-bottom"><hr>
  <h5 class="text-center">Customers List</h5>
                ...
                <?php 
                 ...
                 $rules ="SELECT  *   FROM users  WHERE user_role='customer' AND status='verified' ORDER BY user_id DESC";
             ...
             //     echo "<td>{$db_user_addresses}</td>";
                  ...
                  echo "<td><a class='btn btn-primary' href='users.php?source=customer&change_role_admin={$db_id}'> Admin</a></td>";
                  echo "<td><a class='btn btn-primary' href='users.php?source=customer&change_role_moderator={$db_id}'>Moderator</a></td>";
                  echo "<td><a  class='btn btn-secondary' href='users.php?source=customer&change_role_customer={$db_id}'>Customer</a></td>";
                  echo "<td><a class='btn btn-primary' href='users.php?source=customer&change_role_delivery_man={$db_id}'>Delivery man</a></td>";
                  ...
                  echo "<td><a  class='btn btn-danger' onClick=\"javascript: return confirm('Are you sure to delete?');  \" href='users.php?source=customer&delete={$db_id}'><i class='fa-solid fa-trash-can'></i></a></td>";
                  echo "</tr>";
              ...
              if(isset($_GET['change_role_admin'])){
                ...
                header("Location: ./users.php?source=customer");
